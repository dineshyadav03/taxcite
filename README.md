...
Phase 1 core pipeline + evaluation built and verified end-to-end (2026-08-10).
...
**Embeddings**: Voyage AI's hosted `voyage-4` model (free tier, 200M tokens), not a local model — moved off local compute entirely per this project's constraint of preferring free hosted resources when local isn't actually needed. Getting here took two false starts, both instructive: Google's Gemini Embedding API looked more generous on paper but has a hard 1,000-requests/day free-tier ceiling (found only in the full error detail after assuming it was a transient rate limit); Voyage AI's true free tier without a payment method on file throttles to 3 RPM / 10K TPM (a payment method would lift this, but entering one is a real ask this zero-budget project deliberately avoids even though usage stays free). The index build is resumable and persists incrementally per batch specifically because of this — a multi-hour build against a 3 RPM ceiling needs to survive interruptions.

**Retrieval**: vector search (ChromaDB + Voyage AI) plus a direct exact-section-number lookup, added after verifying the core failure mode by hand — a generic query like "what does section 194 say" put the true Section 194 at high cosine distance, while an unrelated section that merely *mentions* "194" in a cross-reference list scored lower and won. Any query naming a section number now gets that section by metadata filter (guaranteed match) before vector search fills remaining slots. Topical queries that don't name a section (e.g. "who has to file a return") still rely on pure semantic similarity and can miss the best section — the same gap [ai-incident-rag](https://github.com/dineshyadav03/ai-incident-rag) solved with hybrid BM25+vector search, not yet ported here.

**Generation**: Groq free tier (llama-3.1-8b-instant), grounded prompt with inline citations, refuses when the top match is a poor semantic fit rather than answering from an irrelevant section. The refusal distance threshold is embedding-model-specific and was recalibrated live against real eval failures (see Evaluation) rather than left as a guess.

**Evaluation** (`eval/golden_qa.json`, `eval/run_eval.py`): 20 hand-written questions with verified ground-truth Act+section answers (drawn from real section titles/content already in the corpus, not assumed from general tax-law knowledge — the 2025 Act in particular has no clean title metadata, so those questions were built by reading actual section text). This is a self-built eval, not an external benchmark — there is no standard public benchmark for a custom income-tax RAG, so "passing the highest" means passing these self-defined, content-verified checks, stated precisely rather than claimed against something that doesn't exist.

- **retrieval_hit_rate: 1.00** — the correct section was in the top-5 retrieved chunks for every question, every time.
- **refusal_accuracy: 1.00** — both genuinely out-of-corpus questions (a cookie recipe, a GST rate — GST is a different Act, not indexed) were correctly refused.
- **citation_accuracy: 0.90** (18/20) — the generated answer explicitly cited the correct section.
- **pass rate: 20/20 (100%)** after one real fix: the first full run scored 17/20, and all 3 failures shared one root cause — retrieval had already found the right section (hit-rate was already 1.00) but the system refused anyway, because the confidence threshold (0.9) was calibrated for a different embedding model before the Voyage AI switch and never re-checked. Recalibrated to 1.15 against the actual observed distance distribution (relevant matches ~0.7-1.15, verified off-corpus queries ~1.2-1.4) and all 3 previously-failing questions were individually re-verified to answer correctly. Full run details in `eval/results.json`.
...
- Table-heavy content (rate schedules, cross-reference tables) doesn't extract cleanly to prose. Verified case: the 2025 Act's Section 194 references a rate table whose rows extracted as repetitive garbled text, which caused the LLM to loop on a nonsense repeated-"194" response in earlier manual testing (the eval run's version of this question passed; degenerate LLM repetition isn't fully deterministic run-to-run, so this remains a real, known risk with that section, not a fixed one).
...
3. ~~**Evaluation**~~ — done (see Status): retrieval-hit-rate, citation-accuracy, and refusal-accuracy against a hand-verified golden set
...
Zero monetary budget — free/local resources only (Voyage AI's free tier for embeddings, Groq's free tier for generation, no payment method entered anywhere).
