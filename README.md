...
11. [Which pattern should you use?](#which-pattern-should-you-use)
12. [Evaluation](#evaluation)
13. [Bugs found and fixed during verification](#bugs-found-and-fixed-during-verification)
...
**Composition.** `search()` runs the exact lookup first; if it returns fewer than `top_k` results, a hybrid of vector similarity and BM25 keyword search (`src/bm25_index.py`) fills the remaining slots via Reciprocal Rank Fusion, deduplicated by chunk ID. The two rankers have different failure modes — vector similarity can miss a lexically distinctive statutory term that doesn't embed close to a colloquial phrasing of the same question, and BM25 alone was verified live to have the opposite problem (common words like "income" dominating a multi-word query and burying the actually-relevant section 24th out of 2,402 in one test) — so neither replaces the other, they cover each other's gap. One safety constraint: vector search's own top hit always leads the non-exact results, never a BM25-only chunk, because every pattern's confidence-refusal gate reads `chunks[0]["distance"]`, and BM25 has no comparable distance value - the hybrid fusion only improves recall in the remaining slots, on top of that unchanged, calibrated gate. Re-run against the golden set after adding this: 20/20 still pass, 100% across all three metrics - no regression. In ad-hoc testing on a few deliberately obscure/technical queries, vector search alone already performed strongly on this corpus (consistent with its already-high measured accuracy), so this is verified as a sound, zero-regression defensive improvement for the documented topical-query gap, not a fix for a frequently-observed failure - stated as measured, not oversold.
...
Thirteen selectable patterns is only useful if it's clear which one to reach for. The table below is not design intent — it is the actual result of asking the identical question ("what are the differences between the 1961 and 2025 Acts?") to all thirteen patterns in one sitting and recording what really happened: real latencies, real refusals, real answers. That test is the evidence behind every claim below, not a description of what each pattern is supposed to do.

| Pattern | Best for | Weak for | Typical latency |
|---|---|---|---|
| **Adaptive** | The default choice when you don't know which pattern fits — reads the question and routes automatically. | Nothing specific — it's only as good as the pattern it routes to. | Fast (routing decision is near-instant, then inherits the routed pattern's time) |
| **Simple** | A question that names an exact section, or a well-phrased topical question with one clear answer. | Broad or comparative questions with no single matching section — it refuses rather than guess. | Fast (well under 1s) |
| **Memory** | Multi-turn conversations — "what about the old Act?" after an earlier answer. | Standalone questions, and the same broad-question weakness as Simple (it shares the same single-shot retrieval). | Fast |
| **Graph** | A question about a section's own dependencies — what it cites, what depends on it. | Same single-shot weakness as Simple on broad questions. | Fast |
| **Self-RAG** | When you want the system to double-check its own answer against the source before returning it. | Same single-shot weakness as Simple; the extra critique pass doesn't help retrieval, only answer faithfulness. | Fast-medium |
| **Multi-modal** | Rate, slab, and threshold questions — anything that lives in a Schedule table rather than prose. | Broad comparative questions; verified live, it refused the same way Simple did. | Fast, except when a table-search retry is triggered (verified live: one run took 42.8s) |
| **Precedent** | A question about one of the 18 curated sections that has real linked Supreme Court case law. | Any section outside that curated set, and the same single-shot retrieval weakness as Simple otherwise. | Fast |
| **Correspondence** | Exactly one job: "what's the 2025-Act equivalent of 1961 Section N?" | Everything else — it explicitly declines rather than answer a question it isn't built for (verified live: this is what it actually does, not a hypothetical). | Fast (no embedding call at all) |
| **HyDE** | Vague or colloquial phrasing far from statute language ("do I pay tax if my friend sends me money"). | Nothing observed in testing — it was one of the few single-pass patterns that succeeded on a broad comparative question too, by searching with a drafted hypothetical passage instead of the literal question. | Fast-medium |
| **Corrective** | A good general-purpose upgrade over Simple — grades what it finds and retries once if the match is weak. | Nothing specific; verified live it handled the same broad comparative question that stumped Simple, Memory, and Graph. | Fast-medium, more if a retry triggers |
| **Branched** | Explicit multi-part or "compare old vs. new" questions — this is the pattern actually built for them. | Simple single-section lookups, where the decomposition step is pure overhead. | Slow (verified live: 87s) |
| **Agentic** | Multi-hop questions needing several fetch/follow-reference steps chosen by the model itself. | Simple lookups, for the same reason as Branched. | Slow (verified live: 72s) |
| **Jury** | A single-answer question where you want a confidence signal — three independent methods agreeing is real evidence. | Broad comparative questions — verified live, it produced a confusing disagreement result here because none of its three jurors (Simple, Corrective, Graph) are built for whole-Act comparison in the first place. | Slow (verified live: ~100-175s — three sequential pattern calls under Voyage's free-tier 3-requests/minute throttle) |

**The pattern behind the pattern:** every one of Simple, Memory, Graph, Self-RAG, Multi-modal, and Precedent shares the same underlying mechanism — one retrieval pass, one confidence check, refuse if the best match isn't good enough. On the live test above, all six converged on the exact same three weak candidate sections and correctly refused rather than answer with low confidence. Branched, Adaptive (by routing to Branched), Agentic, Corrective, and HyDE each do something beyond a single retrieval pass — decompose, loop, retry, or search with a different query — and that's what let them actually answer a question none of the single-pass patterns could. Speed and capability trade off directly: the patterns that do more reasoning steps take longer, by a wide margin (roughly 40-100x slower in this test), because each extra step under a free-tier rate limit costs real wall-clock time, not just a marginally bigger prompt.
...
- **Topical queries that never name a section number now get hybrid BM25-plus-vector retrieval** (see [Retrieval mechanics](#retrieval-mechanics)) rather than vector search alone — the exact-lookup fast path still only triggers when a query names a number explicitly, but the fallback path is no longer single-signal. Verified live: no regression on the golden set (still 20/20), and the two rankers were confirmed to have different failure modes in ad-hoc testing. Not yet observed to flip a previously-wrong answer to right in this project's own testing - a defensive improvement to a documented gap, not a fix for an observed frequent failure.
